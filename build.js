...
async function build() {
  ...
  for (const browser of browsers) {
    ...
    const manifestSource =
      browser === "chrome" ? "manifest.chrome.json" : "manifest.firefox.json";
    await fs.copy(
    ...
    if (await fs.pathExists(path.join(srcDir, "utils"))) {
      await fs.copy(path.join(srcDir, "utils"), path.join(distDir, "utils"));
    }
    ...
    if (await fs.pathExists(path.join(__dirname, "icons"))) {
      await fs.copy(path.join(__dirname, "icons"), path.join(distDir, "icons"));
    }
    ...
    if (await fs.pathExists(peerjsSource)) {
      await fs.copy(peerjsSource, path.join(distDir, "lib", "peerjs.min.js"));
    }

    console.log(`✅ ${browser} build complete at ${browser}/\n`);
  }

  console.log("🎉 Build completed successfully!");
  console.log("\nNext steps:");
  console.log("  Chrome: Load unpacked extension from chrome/");
  console.log("  Firefox: Load temporary add-on from firefox/");
}
